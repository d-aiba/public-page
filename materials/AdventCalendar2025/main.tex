...
\begin{itemize}
	...
	\begin{align*}
		...
		\mu_X\left( \sum_{i\in I} a_i \Ket{\sum_{x\in X} b_{ix} \Ket{x}} \right)=\sum_{x\in X} \left( \sum_{i\in I} a_i b_{ix} \right) \Ket{x}
	\end{align*}
	例えば \begin{small}$\displaystyle \left(\frac{1}{3}\Ket{\frac{1}{2}\ket{x}+\frac{1}{2}\ket{y}}+\frac{2}{3}\Ket{\ket{z}}\right)\mapsto \left(\frac{1}{6}\ket{x}+\frac{1}{6}\ket{y}+\frac{2}{3}\ket{z}\right)$\end{small} となり、2段階の遷移を1段階に潰します。
	%%%
	\begin{center}
		\begin{small}
			\begin{tikzcd}[row sep=0pt,column sep=18pt]
...
さて、モナドの中でも「強モナド」と呼ばれるものがあります。%次ページで紹介します。
