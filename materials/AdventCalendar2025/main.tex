...
\noindent 
ここで、モナド、クライスリ圏、アイレンバーグ・ムーア圏を考えることのモチベーションをつかむために、例として、\textgt{代数的エフェクト} (algebraic effect)、とりわけ\textgt{例外処理} (exception handler) の例を見ておくことにしましょう。\vspace{18pt}
\begin{itemize}
	\item まず、例外モナド (exception monad) は以下のように与えられます。
	\begin{itemize}
		\item $T(X):=X+E$ と定めます。ここで $+$ は余積とし、$E$ は ``エラーの集合'' とします。\\ さらに、$f:X\to Y$ については、$T(f)=f+\id_E$ とします。
		...
		\begin{multicols}{2}
			...
			\end{center}
		\end{multicols}
		\item また、乗法 $\mu_X$ を以下のように定義します。（「エラーが起きたら、そのエラーを送って、最後に集める。」）
		\begin{multicols}{2}
			\begin{center}
	...
	\end{itemize}
	\item 次に、$f: X\to Y+E$ と $g: Y\to Z+E$ に対し、それらのクライスリ合成 (Kleisli composition) は \[g\ast f:=\mu_Z\circ Tg\circ f\] で定まります。（「もしエラーが起きたら、そのエラーを送って、次の計算はスキップする。もしエラーが起きなければ、その計算結果を送ってから、そのまま次の計算も続行する。」）
	\begin{align*}
		...
		\end{array}
	\end{align*}
	\item さらに、このモナド $(T,\eta,\mu)$ のもとで、$X+E\xrightarrow{h} X$ を「例外処理」(exception handler) と呼びます。\\これは $T$-代数をなします。%（代数的エフェクトと呼ばれるもののうちの一種です。）
	...
	によって「各例外 $e$ に対して、回復値 $k(e)\in X$ を選ぶこと」と実質的に同じです。
\end{itemize}
...
\noindent 
ちょうどいい機会なので、「\textbf{前モノイダル直積}」(pre-monoidal product) という考え方にも触れておきましょう。
...
\begin{itemize}
	...
	\item このとき、$FTX=R^T(F^T(L^T(X)))$ となります。%\\ 任意の対象 $X$ に対し、自由 $T$-代数とは、$L^T(X)=\left(TTX\xrightarrow{\;\mu_X\;}TX\right)$ です。（台集合が$TX$）\\ %ここで、$FTX$ の自由 $T$-代数 $\alpha:T(FTX)\to FTX$ を考え、 \[\delta=\] を考えます。
	\begin{itemize}
	...
	\end{itemize}
	% いま、
	\item すなわち、$\gamma:X\to FTX$ は、実は $\gamma:X\to R^T(F^T(L^T(X)))$ という射だと言えます。
...
このように、モナド分配則はあると便利なのですが、実際には、都合よくモナド分配則がいつもあるとは限りません。\\以下ではそのことについて、簡単な例をもとに考えてみましょう。（以下に挙げるような例は、自然変換にならないため、したがってモナド分配則の候補にもなれないことを確認しましょう。）
