...
以下では、「モナド」と「クライスリ圏」そして「アイレンバーグ・ムーア圏」をそれぞれ導入します。（これらの考え方が、後の章を理解するうえでも役立つという意図で、また、関連する周辺事項にも入門しやすくなるという意図で、本節を設けています。これらを念頭に置くことによって、次章の背後にある問題設定や動機づけをより明確に理解できるようになるはずです。）

なお、この節で扱う内容は、Haskel などのプログラム言語にも用いられています。モナドとクライスリ圏の考え方は、「計算効果」を関数型言語の中で扱うための仕組みのひとつとして知られています。（Haskel のモナドに関する話題についてはこの記事では一切触れませんが、気になる人はぜひ自ら勉強してみてください。）
...
\begin{definition}[モナド]
	...
	\begin{multicols}{2}
		...
		\begin{center}
			\begin{tikzcd}
				TX \ar[r,"\eta_{{}_{T\!X}}"] & TTX \ar[r,leftarrow,"T\eta_X"] \ar[d,"\mu_X"] & TX \\
				{} & TX \ar[lu,equal] \ar[ru,equal] & 
			\end{tikzcd}
		\end{center}
	\end{multicols}
\end{definition}
