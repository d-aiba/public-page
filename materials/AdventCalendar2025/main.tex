...
%----------
%----------
\title{述語変換子意味論と余代数\\\Large{圏論を用いたモデル検査入門}}
...
\subsection{余談 (2): モナドとクライスリ圏}

通常、プログラム意味論において副作用（非決定性や確率など）を伴う計算は、モナド $F$ を用いた写像 $c:X\to FY$ として表されます。これは、クライスリ圏 $\Kl(F)$ における射 $c:X\computationrightarrow Y$ とみなすこともできます。
...
以下では、「モナド」と「クライスリ圏」をそれぞれ導入します。（これらの概念そのものは後の章で直接的に使うわけではありませんが、これらの考え方が後の章を理解するうえでも役立つという意図で、また、関連する周辺事項にも入門しやすくなるという意図で、本節を設けています。これらを念頭に置くことで、次章の背後にある問題設定や動機づけをより明確に理解できるようになるはずです。）
...
\begin{example}[分布モナド]
	...
	\begin{itemize}
		\item $\mathcal D:\Set\to\Set$ は分布関手である。（\textbf{例\ref{ex:MarkovChain}}を参照）
		\begin{align*}
...
\begin{remark}
	同一の射の上にあるカルテシアン射は ``同型を除いて一意'' (unique up to iso) である。より正確に言うと、ファイブレーション $p:\E\to\B$ と、$\B$ の射 $u:I\to J$ が与えられ、$Y\in\E_J$ があり、2つの $p$-カルテシアン射 $\overline{u}_Y:u^\ast Y\to Y$ と $f:X\to Y$ がいずれも $u$ の上にあるとき、一意な同型射 $\alpha:X\xrightarrow{\cong} u^\ast Y$ がファイバー $\E_I$ に存在し、$f=\overline u_Y\circ\alpha$ が成り立つ。（証明略）%\vspace{3pt}
