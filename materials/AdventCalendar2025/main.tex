...
\textbf{ホーア論理} (Hoare logic) と\textbf{述語変換子} (predicate transformer) は、プログラムの正しさ（意図した挙動をするか、バグがないかどうか）を数学的に検証するための異なる2つのアプローチです。プログラムが満たしていてほしい性質（仕様）を論理式で記述し、それが満たされているかを論理学的手法を用いて確かめます。
...
$N$に正の自然数を入力してこのプログラムを実行した後に、事後条件「$k=N!$」を満たしてほしいものとします。ループが何回実行されるかわからない中で、\kenten{あらゆる入力} $N$ \kenten{に対してどうやって保証}すればよいでしょうか。答えは、\uwave{毎回のループを通過する前後で（各変数の値が変わっても）常に保存される性質（\textbf{ループ不変量}）}「$k\cdot n!=N!$」に注目する、というアイデアです。そうすることで「階乗を出力する」という仕様が必ず満たされることを保証できます。つまり、事後条件を保証するためには、ループ不変量を探せばよいのです。これが\textbf{ホーア論理}の考え方です。\vspace{5pt}

では今度は、この事後条件「$k=N!$」を成立させるためには、\kenten{事前条件として最低限どんな条件を課す必要があるか}、という問題を考えてみましょう。答えは、「$N\geq 0$」です。これを知るには、\uwave{事後条件から出発してプログラムを} \uwave{後ろから前へ}\uwave{たどり、各計算に応じて条件を置き換えていく}という操作をする必要があります。ただし、ループが何回実行されるかわからないため、この操作の不動点をとります。これが\textbf{述語変換子}、とりわけ\textbf{最弱事前条件}の考え方です。
...
このように、モナド分配則はあると便利なのですが、実際には、都合よくモナド分配則がいつもあるとは限りません。\\以下ではそのことについて、簡単な例をもとに考えてみましょう。（以下に挙げるような例は、自然変換にならないため、したがってモナド分配則の候補にすらなれないことを確認しましょう。）
