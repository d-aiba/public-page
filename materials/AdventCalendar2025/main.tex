...
典型例としては、例外処理 (exception handling) と呼ばれる計算効果を表す関手 $F(Y)=Y+E$ を用いた、「効果つきの計算」$f:X\to Y+E$ が挙げられます。ここで $+$ は余積を表し、$E$ は ``エラーの集合'' とみなしましょう。このとき、計算 $f:X\to Y+E$ は、単に入力値 $x\in X$ から出力値 $y\in Y$ を返すだけでなく、「もし例外が生じたらエラー $e\in E$ を返す」という副作用 (side effect) を持ちます。
...
以下では、「モナド」と「クライスリ圏」をそれぞれ導入します。（これらの概念そのものは後の章で直接的に使うわけではありませんが、これらの考え方が後の章を理解するうえでも役立つという意図で、本節を設けています。これらを念頭に置くことで、次章の背後にある問題設定や動機づけをより明確に理解できるようになるはずです。）

なお、この節で扱う内容は、Haskel などのプログラム言語にも用いられています。モナドとクライスリ圏の考え方は、「計算効果」を関数型言語の中で扱うための仕組みのひとつとして知られています。（Haskel のモナドに関する話題についてはこの記事では一切触れませんが、気になる人は自ら勉強してみてください。）
...
\begin{remark}
	モナドの$\mu$は、2段階の分岐を1段階に潰す役割を果たします。（後ほど例を交えて説明します。）\vspace{5pt}
	\begin{center}
...
\begin{example}[分布モナド]
	...
	\begin{itemize}
		...
		\begin{align*}
			\mathcal D(X)&=\{d:X\to [0,1]\mid\textrm{finite support,~}\textstyle\sum d(x)=1\}\\&=\{\textstyle\,\sum p_i\ket{x_i}\,\textrm{, where each $p_i$ is a probability $d(x_i)\in[0,1]$.}\}\\\mathcal D(f)&:\mathcal D(X)\to\mathcal D(Y)\\&\mathcal D(f)(\textstyle\sum p_i\ket{x_i})=\textstyle\sum p_i\ket{f(x_i)}
		\end{align*}
	...
	\begin{small}
		...
		\begin{center}
			\begin{tikzcd}
				...
				\Pred \ar[loop left,"\dot F"] \ar[dd,"p"'] &&& \dot F P \ar[lld,mapsto,shorten=3pt] \\[-24pt] 
				& c^\ast\dot F P & \\
				~\Set~ \ar[loop left,"F"]  & X \ar[rr,"c"'] && FX 
			\end{tikzcd}
		\end{center}
...
本節では、「最弱事前条件意味論の圏論的定式化が実際にどのようにモデル検査に役立つか」を紹介するという目的で、確率的プログラムの\textbf{停止性} (almost-sure termination) および\textbf{期待実行時間} (expected runtime) の検証に関して、筆者が最近行った研究内容を（少しだけ）取り上げます。
