...
典型例としては、例外処理 (exception handling) と呼ばれる計算効果を表す関手 $F(Y)=Y+E$ を用いた、「効果つきの計算」$f:X\to Y+E$ が挙げられます。ここで $+$ は余積を表し、$E$ は ``エラーの集合'' とみなします。このとき、計算 $f:X\to Y+E$ は、単に入力値 $x\in X$ から出力値 $y\in Y$ を返すだけでなく、「もし例外が生じたらエラー $e\in E$ を返す」という副作用 (side effect) を持ちます。
...
以下では、「モナド」と「クライスリ圏」をそれぞれ導入します。（これらの概念そのものを後の章で直接的に使うわけではありませんが、これらの考え方が後の章を理解するうえで役立つ、という意図で、本節を設けています。これらを念頭に置くことで、次章の背後にある問題設定や動機づけをより明確に理解できるようになるはずです。）

なお、この節で扱う内容は、Haskel などのプログラム言語にも用いられています。モナドとクライスリ圏の考え方は、「副作用」を関数型言語の中で扱うためのメカニズムとして知られています。この記事では Haskel のモナドに関する話題などには触れませんが、気になる人は自ら勉強してみてください。
