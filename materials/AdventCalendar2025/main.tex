...
さて、本章でここまで扱ってきた述語変換子の考え方は、圏論の言葉を使うととても見通しがよくなります。そのための準備として、次章では、圏論の関手を使ってプログラムの振舞いを捉える方法について紹介します。（すでに「余代数」や「モナド」や「グロタンディーク・ファイブレーション」のことは知っているよ、という人は、3章へ飛んでください。）
...
以下では、「\textbf{モナド}」と「\textbf{クライスリ圏}」そして「\textbf{アイレンバーグ・ムーア圏}」を導入します。（これらの考え方は、後の章で直接的に使うというわけではありませんが、次章の内容の背後にある問題設定や動機づけをより明確に理解するうえで役立つという意図で、また、関連する周辺事項にも入門しやすくなるという意図で、本節を設けます。）\vspace{6pt}

なお、この節で扱う内容は、Haskell などのプログラム言語にも用いられています。モナドやクライスリ圏の考え方は、「計算効果」を関数型言語の中で扱うための仕組みとして知られています。（ちなみに、Haskel におけるモナドなどに関する話題についてはこの記事では触れませんが、気になる人はぜひこの機会に自ら勉強してみてください。）
...
\begin{definition}[クライスリ圏]
	$T:\C\to\C$ を、圏 $\C$ 上の自己関手とする。モナド $(T,\eta,\mu)$ が与えられたとき、以下のように定められた圏 $\C_T$ を、\textbf{クライスリ圏} (Kleisli category) と呼ぶ。
	\begin{itemize}
...
\begin{definition}[EM圏]
	$T:\C\to\C$ を、圏 $\C$ 上の自己関手とする。モナド $(T,\eta,\mu)$ が与えられたとき、以下のように定められた圏 $\C^T$ を、\textbf{アイレンバーグ・ムーア圏} (Eilenberg-Moore category) と呼ぶ。
	\begin{itemize}
...
\noindent ここで、「\textbf{前モノイダル直積}」(pre-monoidal product) という考え方に触れておきましょう。
...
~~ ここで、クライスリ圏 $\C_T$ における直積が、一般には「モノイダルな直積」ではなく、「前モノイダルな直積」をなすことに注意が必要です。具体的に言うと、圏 $\C_T$ における直積の構成
\begin{center}
	\begin{tikzcd}[row sep=-2mm,column sep=15mm]
		X \ar[r,"f"] & X' & \fbox{in $\mathrm{Kl}(T)$}\\
		\otimes & \otimes \\[2pt]
		Y \ar[r,"g"] & Y'
	\end{tikzcd}
\end{center}
は、\kenten{射を適用する順番に左右されます}。というのも、
\begin{align*}
...
\end{align*}
の両者は異なります。このように、クライスリ圏における直積は、射を適用する順番に敏感なのです。

~~ さて、クイズです。クライスリ圏の\kenten{直積がモノイダルになるのはどんなとき}でしょうか？（考えてみてください。）答えは、$T$ が\kenten{可換な強モナドであるとき}です！ つまり、二重ストレングス $T(X)\otimes T(Y)\to T(X\otimes Y)$ があるとき、クライスリ圏 $\C_T$ はモノイダルになります。（すなわち、ストリング図式で書ける！）
\begin{align*}
	\begin{array}{cccccc}
		f\otimes g &=& (\id\otimes g)\circ(f\otimes\id) &=& (f\otimes\id)\circ(\id\otimes g) & \qquad\fbox{in $\Kl(T)$}\\[8pt]
	...
	\end{array}
\end{align*}
これはなぜかというと、$T$ が可換な強モナドであるとき、\vspace{3pt}
\begin{center}
...
\begin{definition}[モナド分配則]
	モナド分配則 (distributive law) とは、2つのモナド $(T,\eta^T,\mu^T)$ と $(S,\eta^S,\mu^S)$ の間の自然変換 $\lambda:TS\Rightarrow ST$ であって以下の図式を可換にするものである。\vspace{8pt}
	% \begin{center}
	% 	\begin{small}
	% 	\begin{tikzcd}
...
\noindent さて、上記で挙げた2種類の異なる見方 \fbox{\textgt{(1) Kleisli approach}} と \fbox{\textgt{(2) Eilenberg-Moore approach}} は、実はそれぞれ
\begin{enumerate}[(1)~]
...
\end{enumerate}
に基づくものだと言うことができます。このことについて、以下で詳しく見ていきましょう。まずは両者の定義から。
...
さてここで、次に、軌跡意味論 (trace semantics) と呼ばれる考え方について紹介します。

\vspace{650pt}

寄り道（余談）はこのくらいにして、次節では、いよいよ「グロタンディーク・ファイブレーション」の話に移りましょう。
...
やループ不変量
を「引き戻し」と「持ち上げ」によって統一的に書くことができるようになります。\vspace{8pt}

ここでは、まず直観を説明してから、後ほど正確な定義を述べます。例として $p:\Pred\to\Set$ というファイブレーションを考えてみましょう。ひとつずつステップを踏んで理解していきます。\vspace{8pt}
...
最後に、「最弱事前条件意味論の圏論的定式化が実際にどのようにモデル検査に役立つか」を紹介するという目的で、確率的プログラムの\textbf{停止性} (almost-sure termination) および\textbf{期待実行時間} (expected runtime) の検証に関して、筆者が最近行った研究内容を（少しだけ）取り上げます。
