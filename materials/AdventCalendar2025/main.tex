...
\begin{remark}
	...
	\begin{center}
	\begin{small}
	\begin{tikzcd}[row sep=2pt,column sep=18pt]
		&& \bullet \\
	...
	\end{tikzcd}
	\quad $\implies$ \quad
		\begin{tikzcd}[row sep=10pt,column sep=38pt]
			& \bullet \\
...
\noindent なお「劣分布」とは、遷移確率が$1$以下の分布のことで、たとえば「$x_0$から、確率$0.4$で$x_1$に遷移し、確率$0.3$で$x_2$に遷移し、それ以外は異常停止 (deadlock) する」といったものです。（これは理論的にも便利で、sub-distribution の集合は $\omega$-cpo になります。）\vspace{10pt}
...
\noindent ※ なお、両者のアプローチ (1) $X\to \mathcal P(L\times X)$ と (2) $X\to \mathcal P(X)^{L}$ は、同じもの（見方を変えただけ）です。\vspace{2pt}%\\
\small 
\begin{prooftree}
...
上記の2種類の異なるアプローチ \fbox{\textgt{(1) Kleisli approach}} および \fbox{\textgt{(2) Eilenberg-Moore approach}} で考えていたものは、以下で導入する \fbox{\textgt{(1) Kleisli extension}} と \fbox{\textgt{(2) Eilenberg-Moore lifting}} にそれぞれ相当します。
