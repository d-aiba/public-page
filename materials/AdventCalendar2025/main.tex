...
%----------
%----------
\title{\textbf{述語変換子意味論と余代数}\\\textbf{\Large{圏論を用いたモデル検査入門}}}
...
では今度は、この事後条件「$k=N!$」を成立させるためには、\kenten{事前条件として最低限どんな条件を課す必要があるか}、という問題を考えてみましょう。答えは、「$N\geq 0$」です。これを知るには、\uwave{事後条件から出発してプログラムを後ろから前へ} \uwave{たどり、各計算に応じて条件を置き換えていく}、という操作をする必要があります。ただし、ループが何回実行されるかわからないため、この操作について不動点をとります。これが\textbf{述語変換子}、とりわけ\textbf{最弱事前条件}の考え方です。
...
\begin{remark}
	...
	この演算子 $\Phi_Q:\mathrm{Pred}(S)\to\mathrm{Pred}(S)$ は単調 (monotone) なので、ナスター・タルスキの定理 (the Knaster-Tarski theorem) より、最小不動点が存在する。
\end{remark}
...
このクリプキフレームで $x_\varepsilon\models\mu f$ が成り立つことが、以下の図のような超限反復からわかります。%（この鎖は、$\omega+1$で安定 (stabilize) し、そこから先は変わらない、つまり最小不動点となります。）
...
\noindent この鎖は、$\omega+1$で安定 (stabilize) し、そこから先は変わらない、つまり $f^{\omega+1}(\bot)$ が最小不動点となります。\vspace{5pt}
...
さて、本章でここまで扱ってきた述語変換子の考え方は、圏論の言葉を使うととても見通しがよくなります。そのための準備として、次章では、圏論の関手を使ってプログラムの振舞いを捉える方法について紹介します。（すでに「余代数」や「グロタンディーク・ファイブレーション」のことはだいたい知っているよ、という人は、3章へ飛んでください。）
...
では、この例のような副作用を持つ計算（効果つき計算）どうしの「合成」はどう考えればいいでしょうか。たとえば、2つの効果つき計算 $f:X\to Y+E$ と $g:Y\to Z+E$ を合成して、$g\circ f:X\to Z+E$ のようなものを作りたいとき、$f,g$の両者は型が合っていないため、一筋縄ではいかないことがわかります。そこで、次節で導入する「モナド」と「クライスリ圏」という概念が役立ちます。\\%（次節でもあらためてこの例外処理の例について取り上げます。）
...
以下では、「\textbf{モナド}」と「\textbf{クライスリ圏}」そして「\textbf{アイレンバーグ・ムーア圏}」を導入します。（これらの考え方は、後の章で直接的に使うというわけではありませんが、次章の内容の背後にある問題設定や動機づけをより明確に理解するうえで役立つという意図で、また、関連する周辺事項にも入門しやすくなるという意図で、本節を設けます。）\vspace{5pt}

なお、この節で扱う内容は、Haskell などのプログラム言語にも用いられています。モナドやクライスリ圏の考え方は、「計算効果」を関数型言語の中で扱うための仕組みとして知られています。（Haskel のモナドに関する話題については、この記事では触れませんが、気になる人はぜひ自ら勉強してみてください。）

\clearpage

\begin{definition}[モナド]
	\textbf{モナド} (monad) とは3つ組 $(T,\eta,\mu)$ であって、以下を満たすものである。\\ 1) $T:\C\to\C$は関手である。2) 単位 $\eta:\id_{\mathbb C}\Rightarrow T$は自然変換である。3) 乗法 $\mu:T\circ T\Rightarrow T$ は自然変換である。\\ そして以下の図式を可換にする。\vspace{5pt}
	% \begin{multicols}{2}
		\begin{center}
			\begin{tikzcd}
...
\begin{remark}
	モナドの$\mu$は、2段階の状態遷移を1段階に潰す役割を果たします。（後ほど例を交えて説明します。）\vspace{5pt}
	\begin{center}
...
\begin{definition}[可換モナド]
	\textbf{可換モナド}とは、\textbf{二重-ストレングス} (double-strength) $T(X)\otimes T(Y)\to T(X\otimes Y)$ を持った強モナドである。二重-ストレングスは、以下の図式のとおり、左右のストレングスから構成される（可換性）。\vspace{5pt}
	\begin{center}
		\begin{tikzcd}[row sep=0pt,column sep=35pt]
...
\begin{example}[モノイド計算効果] \label{ex:monoid-computational-effect}
	...
	\begin{itemize}
		...
		\item モナドの積 $(M\otimes M)\otimes X\xrightarrow{\mu_X} M\otimes X$ は、%以下で与えられる。
		...
		\end{multicols}
	\end{itemize}
	ここで、モノイド $M$ を「副作用」(side effect) と呼ぶ。さて、$(T,\eta,\mu)$ がモナドをなすことの恩恵は2つある。
	\begin{enumerate}
		% \begin{multicols}{2}
...
\begin{example}[writerモナド]
	ライターモナド (writer monad) $T=\Sigma^\ast\times(-)$とは、\textgt{例\ref{ex:monoid-computational-effect}}が非可換モナドの場合である。 
\end{example}
...
\begin{definition}[可換な強モナド]
	
\end{definition}

\begin{remark}
	冪集合モナド $\mathcal P$ (powerset monad) や分布モナド $\mathcal D$ (distribution monad) は可換である。\\ 他方で、たとえば継続モナド $\Omega^{\Omega^{(-)}}$ (continuation monad) は可換ではない。
\end{remark}
...
\begin{definition}[モナドの分配則]
	モナドの分配則 (distributive law) とは、2つのモナド $(T,\mu^T,\eta^T)$ と $(S,\mu^S,\eta^S)$ の間の自然変換 $\lambda:TS\Rightarrow ST$ であって以下の図式を可換にするものである。\vspace{8pt}
	% \begin{center}
	% 	\begin{small}
	% 	\begin{tikzcd}
