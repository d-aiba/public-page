...
\begin{itemize}
	...
	\item （解決策） ここで、様相 $\tau:F\Omega\to\Omega$ が重要な役割を果たします。この $\tau$ は、「\uwave{$Y$上の述語}」から「\uwave{$FY$上の述語}」へと持ち上げる操作、\textbf{述語持ち上げ} (predicate lifting) を誘導します。\\具体的には、各 $Y$ について、\[\dot F_Y:\Omega^Y\to\Omega^{FY},\quad \dot F_Y(q):=\tau\circ Fq\qquad \textrm{(ただし $q:Y\to\Omega$ を任意の述語とする)}\] と定めます。この $\dot F_Y$ は、「$Y$上の述語$q$を、計算作用$F$で包まれた状態空間$FY$上の述語 $\dot F_Y(q)$ へと持ち上げる」操作となります。（通常、$\tau$ には単調性を課し、$\dot F_Y$ が単調になるようにします。）\vspace{10pt} \\
	...
	\end{align*}
	となります。つまり、\\(1) $Y$ 上の述語 $q$ を、\uwave{様相 $\tau$ を用いて} $FY$ 上に\kenten{持ち上げて}、\\(2) それを \uwave{計算 $c$ に沿って} $X$ 上に\kenten{引き戻す}、\\という2段階で理解できます。
	% \begin{small}
		% \begin{center}
		% 	\begin{tikzcd}
		% 		\Pred \ar[loop left,"\dot F"] \ar[d,"p"'] & \Omega^{X} \ar[rr,bend left=12pt,"\dot F_X"] && \Omega^{FY} \ar[ll,bend left=12pt,"c^\ast"] \\
		% 		~\Set~ \ar[loop left,"F"]  & X \ar[rr,"c"'] && FY 
		% 	\end{tikzcd}
		% \end{center}
		%%%
	% 	\begin{center}
	% 		\begin{tikzcd}
	% 			& \Omega^{Y} \ar[d,"\substack{\rotatebox{90}{$\in$}}",phantom] && \Omega^{FY} \ar[d,"\rotatebox{90}{$\in$}",phantom] \\[-8pt]
	% 			\Pred \ar[loop left,"\dot F"] \ar[d,"p"'] & q \ar[rr,mapsto,shorten=10pt] && \dot F_Y(q) \\
	% 			~\Set~ \ar[loop left,"F"]  & Y \ar[rr] && FY 
	% 		\end{tikzcd}
	% 	\end{center}
	% 	%%%
	% 	\begin{center}
	% 		\begin{tikzcd}
	% 			& \Omega^{X} && \Omega^{Y} \ar[d,"\substack{\rotatebox{90}{$\in$}}",phantom] && \Omega^{FY} \ar[d,"\rotatebox{90}{$\in$}",phantom] \\[-8pt]
	% 			& && q \ar[rrd,mapsto,shorten=10pt] && {} \\[-15pt]
	% 			& && && \dot F_Y(q) \ar[dllll,mapsto] \\[-15pt]
	% 			\Pred \ar[loop left,"\dot F"] \ar[d,"p"'] & wp\llbracket c\rrbracket(q) && {} && {} \\
	% 			~\Set~ \ar[loop left,"F"]  & X && Y \ar[rr] && FY 
	% 		\end{tikzcd}
	% 	\end{center}
	% % \end{small}
	% \begin{tikzcd}[column sep=huge,row sep=large] 
	% 	\Omega^Y \arrow[r,"\dot F_Y"] \arrow[rr,bend left=30pt,"wp\llbracket c \rrbracket"] \arrow[d,"p"'] & \Omega^{FY} \arrow[d,"p"] \arrow[r,"c^{\ast}"] & \Omega^{X} \arrow[d,"p"] \\ Y \arrow[r,"F"'] & FY & X \arrow[l,"c"] 
	% \end{tikzcd} \quad 
	% where 
	% $\dot F_Y(q)=\tau\circ Fq$ is a predicate lifting. (???)
	% \item 
\end{itemize}
