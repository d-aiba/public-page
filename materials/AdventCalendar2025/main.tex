...
\begin{definition}[置換]
	$p:\E\to\B$ をファイブレーションとし、$u:I\to J$ を $\B$ の射とします。$u$ の置換 (substitution) とは、関手 $u^\ast\colon\E_J\to\E_I$ であって以下を満たすものである。（ここで、$\overline u_X\colon u^\ast X\to X$ は$p$-カルテシアン射で $p(\overline u_X)=u$ を満たすものとする。）\vspace{3pt}
	\begin{center}
...
\begin{remark}
	...
	を満たす。
	\begin{itemize}
		\item 前者ゆえに、$\dot F_X:\E_X\to\E_{FX}$ を定義できることが従う。
		\item 後者は、「関手の持ち上げは置換と整合的である」ことを意味する。
	\end{itemize}
\end{remark}
