...
\begin{itemize}
	\item （状況設定） ファイブレーション $p:\Pred\to\Set$ を考えます。各ファイバー $\Pred_X=\Omega^X$ は「各状態集合 $X$ 上の述語全体」であり順序集合をなし、各 $f:X\to Y$ に対する置換 $f^\ast:\Omega^Y\to\Omega^X,\,q\mapsto q\circ f$ は「事後条件 $q$ を $f$ によって引き戻した事前条件 $f^\ast(q)$ を返す写像」です。 \vspace{10pt}
...
\begin{thebibliography}{99}
	\item 蓮尾 (2011) 「Hoare論理によるプログラム検証」（スライド資料）, ......
    ...
    \item Hasuo, I. (2025) \textit{Abstract and concrete model checking}, (lecture notes for Marktoberdorf summer school 2025), ...... 
\end{thebibliography}
%-----
