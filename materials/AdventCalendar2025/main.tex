...
%----------
%----------
\title{\textbf{述語変換子意味論と余代数\\\Large{圏論を用いたモデル検査入門}}}
...
\begin{proposition}
	$(L,\sqsubseteq)$ を完備束とし、$f:L\to L$ を単調写像とする。このとき、以下の帰結関係（上から下）が成り立つ。\vspace{5pt}
	\begin{center}
	\begin{prooftree}
		\AXC{$p\sqsubseteq i$} 
...
\noindent 
上記の命題の中の、$p$ は事前条件、$q$ は事後条件、$i$ はループ不変量に、それぞれ相当しています。
% ここで、
% \begin{itemize}
...
以下では、「\textbf{モナド}」と「\textbf{クライスリ圏}」そして「\textbf{アイレンバーグ・ムーア圏}」を導入します。（これらの考え方が、後の章を理解するうえでも役立つという意図で、また、関連する周辺事項にも入門しやすくなるという意図で、本節を設けます。これらを念頭に置くことで、次章の背後にある問題設定や動機づけをより明確に理解できるようになるはずです。）\vspace{5pt}

なお、この節で扱う内容は、Haskell などのプログラム言語にも用いられています。モナドとクライスリ圏の考え方は、「計算効果」を関数型言語の中で扱うための仕組みのひとつとして知られています。（Haskel のモナドに関する話題についてはこの記事では一切触れませんが、気になる人はぜひ自ら勉強してみてください。）
...
\begin{example}[モノイドによる計算効果] \label{ex:monoid-computational-effect}
	...
	\begin{itemize}
		...
		\end{multicols}
	\end{itemize}
	ここで、モノイド $M$ を「副作用」(side effect) と呼ぶ。この $(T,\eta,\mu)$ がモナドをなすことの恩恵は2つある。
	\begin{enumerate}
		% \begin{multicols}{2}
...
\begin{example}[分布モナド]
	...
	\begin{itemize}
		...
		\end{align*}
		\item The unit $\eta_X:X\to T(X)$ is defined as \[\eta_X(x)=1\cdot\Ket{x}.\]
		\item The multiplication $\mu_X:T(T(X))\to T(X)$ is defined as \[\mu_X\left( \sum_{i\in I} a_i \Ket{\sum_{x\in X} b_{ix} \Ket{x}} \right)=\sum_{x\in X} \left( \sum_{i\in I} a_i b_{ix} \right) \Ket{x}.\]
	\end{itemize}
\end{example}
