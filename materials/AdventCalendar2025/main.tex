...
以下では、「\textbf{モナド}」と「\textbf{クライスリ圏}」そして「\textbf{アイレンバーグ・ムーア圏}」をそれぞれ導入します。（これらの考え方が、後の章を理解するうえでも役立つという意図で、また、関連する周辺事項にも入門しやすくなるという意図で、本節を設けています。これらを念頭に置くことによって、次章の背後にある問題設定や動機づけをより明確に理解できるようになるはずです。）
...
\begin{example}[モノイド計算作用]
	...
	\begin{enumerate}
		...
		\begin{center}
			...
			\end{tikzcd}
		\end{center}
		\item 計算作用 $X\to TY$ と $Y\to TZ$ を集約して、$X\to TZ$ に合成することができる。
		\begin{center}
		...
		% \end{multicols}
	\end{enumerate}
	なお、$T=M\otimes(-)$ が\kenten{可換モナド}であるのは、$M$ が\kenten{可換モノイド}であるとき、かつそのときに限る。
	\begin{center}
		% \begin{footnotesize}
		% \begin{tikzcd}
