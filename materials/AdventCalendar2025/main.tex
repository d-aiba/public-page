...
なお、この節で扱う内容は、Haskell などのプログラム言語にも用いられています。モナドやクライスリ圏の考え方は、「計算効果」を関数型言語の中で扱うための仕組みとして知られています。（ちなみに、Haskel におけるモナドに関する話題についてはこの記事では触れませんが、気になる人はぜひ自ら勉強してみてください。）
...
\begin{definition}[強モナド]
	...
	同様に、$(\C,\otimes,I)$ 上の\textbf{右-強モナド} (right-strong monad) とは、\textbf{右-ストレングス} (right-strength) 
...
\begin{definition}[モナド分配則]
	モナド分配則 (distributive law) とは、2つのモナド $(T,\mu^T,\eta^T)$ と $(S,\mu^S,\eta^S)$ の間の自然変換 $\lambda:TS\Rightarrow ST$ であって以下の図式を可換にするものである。\vspace{8pt}
	% \begin{center}
	% 	\begin{small}
	% 	\begin{tikzcd}
...
上記の2種類の異なるアプローチ \fbox{\textgt{(1) Kleisli approach}} および \fbox{\textgt{(2) Eilenberg-Moore approach}} で考えていたのは、以下で導入する \fbox{\textgt{(1) Kleisli extension}} と \fbox{\textgt{(2) Eilenberg-Moore lifting}} にそれぞれ相当します。
...
やループ不変量
を「引き戻し」と「持ち上げ」によって統一的に書けます。\vspace{8pt}
...
さて、本節では、非決定的プログラムや確率的プログラムなどといった様々な種類のプログラムに対する述語変換子意味論を得る方法について、圏論的な視座から考えます。一般に、状態の集合 $X, Y$ と真理値の集合 $\Omega$ が与えられたときに、効果付き計算 $c:X\to FY$ に基づいて、事後条件 $q:Y\to\Omega$ を最弱事前条件 $wp\llbracket c\rrbracket(q):X\to \Omega$ に変換する関数である述語変換子 $wp\llbracket c\rrbracket:\Omega^Y\to\Omega^X$ を手に入れるために、どんな手続きを経たらよいでしょうか。
...
\begin{itemize}
	...
	\\ （随伴がないと不便な場面がいろいろあるのですが、対応策としては、米田埋め込みを使う方法があります。詳細は省略しますが、ざっくりとしたアイデアとしてはおおむね以下の図の通りです。）
	\begin{center}
		% \hspace*{-50pt}
