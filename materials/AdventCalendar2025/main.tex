...
\begin{itemize}
	...
	\end{prooftree} \vspace{5pt}
	\item $f:X\to\mathcal PY$ に対し、\\ 述語変換子 $f^\ast$ は様相 $\tau$ の選び方に拠って決まります。\\ $\Box_f(-):2^Y\to 2^X,\,q\mapsto\{x\in X\mid \forall y\in Y\,(x\to y\textrm{ implies } y\in q)\}$ \\ $\Diamond_f(-):2^Y\to 2^X,\,q\mapsto\{x\in X \mid \exists y\in Y \, ( x\to y \textrm{ and } y\in q)\}$ \vspace{5pt}
...
\begin{itemize}
	...
	\hspace*{50pt}
	\end{center}
	\item さて、述語変換子 $f^\ast:[0,1]^Y\to[0,1]^X$ をどうやって手に入れればよいでしょうか？\vspace{5pt} \\ 以下のようにして得られます。（上段が入力、下段が出力です。）\vspace{5pt} 
	\begin{prooftree}
	...
	\end{prooftree} \vspace{5pt}
	\item $f:X\to\mathcal PY$ に対し、述語変換子 $f^\ast:[0,1]^Y\to[0,1]^X$ は以下で定義されます。 \[f^\ast(q)(x)=\sum_{x'\in X}f(x)(x')\cdot q(x')\] すなわち、$f^\ast(q)$ は、各 $x$ に対し、$q$ による値の期待値 $p_1 \cdot q(x_1) + p_2 \cdot q(x_2) + \cdots$ を割り当てます。
	\begin{center}	
		\begin{small}
			\begin{tikzcd}[row sep=8pt]
				& \circled{$x_1$} \\[-3pt]
		...
		\end{small}
	\end{center}
	\item なお、この $f^\ast$ は、左右いずれの随伴も持ちません。\\ （対応策としては、以下のしかたで米田埋め込みを使う方法があります。詳細は省略します。）
	\begin{center}
		% \begin{small}
