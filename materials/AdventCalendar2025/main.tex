...
\begin{enumerate}
	...
	\end{center}
	\clearpage
	\item 自己関手 $F:\Set\to\Set$ と、ファイブレーション $p:\Pred\to\Set$ について、\\ 関手 $\tilde{F}:\Pred\to\Pred$ が、$p$ に沿った $F$ の「持ち上げ」(fibred lifting) であるとは、
	\begin{center}
		\begin{small}
			\begin{tikzcd}
				\Pred \ar[r,"\tilde F"] \ar[d,"p"'] & \Pred \ar[d,"p"] \ar[ld,"\rotatebox{45}{$=$}",phantom]\\
				\Set \ar[r,"F"'] & \Set
			\end{tikzcd} 
			\quad かつ \quad
			\begin{tikzcd}[column sep=28pt]
				\Pred_Y \ar[r,"f^\ast"] \ar[d,"\tilde F"'] & \Pred_X \ar[d,"\tilde F"] \ar[ld,phantom,"\rotatebox{40}{$\cong$}"',shorten=12pt] \\
	...
	\begin{small}
		\begin{center}
			\begin{tikzcd}
				\Pred \ar[loop left,"\tilde F"] \ar[d,"p"'] & \Pred_{X} \ar[rr,bend left=12pt,"\tilde F_X"] && \Pred_{\,FX} \ar[ll,bend left=12pt,"c^\ast"] \\
...
\noindent 以下、ファイブレーションの正確な定義を（念のため）書いておきますが、上記の $p:\Pred\to\Set$ の直観さえきちんと押さえておけば、次節以降の内容を理解するうえで基本的には問題ないです。
