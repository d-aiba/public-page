...
通常、プログラム意味論において副作用（非決定性や確率など）を伴う計算は、モナド $F$ を用いた写像 $c:X\to FY$ としてモデル化されます。これは、クライスリ圏 $\Kl(F)$ における射 $c:X\computationrightarrow Y$ とみなすこともできます。
...
なお、この節で扱う内容は、Haskel などのプログラム言語にも用いられています。モナドとクライスリ圏の考え方は、「計算効果」を関数型言語の中で扱うための仕組みのひとつとして知られています。Haskel のモナドに関する話題についてはこの記事では触れませんが、気になる人は自ら勉強してみてください。
...
\begin{itemize}
	...
	\\ （随伴がないと不便な場面がいろいろあるのですが、対応策としては、米田埋め込みを使う方法があります。ここでは詳細は省略しますが、ざっくりとしたアイデアとしてはおおむね以下の図の通りです。）
	\begin{center}
