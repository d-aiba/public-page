...
\begin{itemize}
	...
	\item すると、以下のように\textbf{合成}を考えることができます。（ここで $X\to TF^2X$ が「\textbf{2ステップ遷移}」を表します。）
	\begin{center}
...
\begin{itemize}
	...
	\item すると、以下のように、非決定的オートマトンを\textbf{決定化} (determinize) することができます。
	\begin{center}
	...
	この $TX\to F(TX)$ を得る過程は、まさに「\textbf{冪集合構成}」(powerset construction) に相当します。\\
	詳しく言うと、以下の合成を通じて、
	\begin{center}
		\begin{tikzcd}[row sep=12pt]
...
\begin{itemize}
	...
	\item これはまさに、先ほどの「2ステップ遷移」そのものです。\vspace{8pt}
\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{align*}
			...
			\end{array}
		\end{align*}
		となり、その台集合（つまり忘却関手 $R^T$ を適用したもの）は $FTX$ です。
		% \item 
		% \item 
	\end{itemize}
	% いま、
	...
	\begin{multicols}{2}
		...
		%%%
		% \begin{center}
			$\gamma^\sharp=\left(TX\xrightarrow{\;(F\mu_X\,\circ\,\lambda_{TX})\,\circ\,T\gamma\;}FTX\right)$
		% \end{center}
	\end{multicols}
