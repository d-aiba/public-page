...
\begin{definition}[可換モナド]
	\textbf{可換モナド}とは、\textbf{二重-ストレングス} (double-strength) $T(X)\otimes T(Y)\to T(X\otimes Y)$ を持った強モナドである。なお、二重-ストレングスとは、以下の図式のように、左右のストレングスから構成されるものとする。（可換性）%\vspace{5pt}
	\begin{center}
...
~~ ここで、クライスリ圏 $\C_T$ における直積が、一般には「モノイダルな直積」ではなく、「前モノイダルな直積」をなすことに注意が必要です。具体的に言うと、圏 $\C_T$ における直積 $f\otimes g$ の構成\vspace{4pt}
\begin{center}
