...
\begin{definition}[モナド]
	モナド (monad) とは3つ組 $(T,\mu,\eta)$ であって、以下を満たすものである。\\ 1) $T:\C\to\C$は関手である。2) $\mu:T\circ T\Rightarrow T$ は自然変換である。3) $\eta:\id_{\mathbb C}\Rightarrow T$は自然変換である。\\ そして 4) 以下の図式を満たす。
	\begin{multicols}{2}
		\begin{center}
			\begin{tikzcd}
		...
		\end{center}
	\end{multicols}
	言い換えると、任意の対象 $X\in\C$ について、$\mu_X:T(T(X))\to T(X)$ と $\eta_X:X\to T(X)$ は以下を満たす。
	\begin{multicols}{2}
		\begin{center}
			\begin{tikzcd}
...
\begin{definition}[モナドの分配則]
	モナドの分配則 (distributive law) とは、2つのモナド $(T,\mu,\eta)$ と $(T',\mu',\eta')$ の間の自然変換 $\lambda:TT'\Rightarrow T'T$ であって以下の図式を可換にするものである。\vspace{5pt}
	\begin{center}
		\begin{small}
		\begin{tikzcd}
