...
\begin{itemize}
	...
	\item 圏論の基本事項（圏の定義、関手、自然変換、極限と余極限、カルテシアン閉圏、など）
\end{itemize}
...
ここで、「\textbf{クライスリ圏}」と「\textbf{アイレンバーグ・ムーア圏}」を導入します。これらは、モナドと密接な関係を持ち、コンピューターサイエンスの中でたくさん使われます。
...
\noindent 
ここで、「\textbf{前モノイダル直積}」(pre-monoidal product) という考え方に触れておきましょう。
...
さて、ここからは、しばらく「モナド分配則」について取り上げます。
...
\begin{definition}[モナド分配則]
	...
	% 	\begin{small}
	% 	\begin{tikzcd}
	% 		TX \ar[r,"T\eta'"] \ar[rd,"\eta'T"'] & TT'X \ar[d,"\lambda"] & T'X \ar[l,"\eta T'"'] \ar[ld,"T'\eta"] \\
	% 		& T'TX 
	% 	\end{tikzcd}
	% 	\end{small}
	% \end{center}
	...
	% 	\begin{center}
	% 		\begin{small}
	% 		\begin{tikzcd}[column sep=8pt,row sep=15pt]
	% 			TT'T'X \ar[rr,"T\mu'"] \ar[d,"\lambda T'"'] && TT'X \ar[d,"\lambda"]\\
	% 			T'TT'X \ar[rd,"T'\lambda"'] && T'TX \\
	% 			& T'T'TX \ar[ur,"\mu'T"'] &
	% 		\end{tikzcd}
	% 		\end{small}
	% 	\end{center}
	...
	% 		\begin{small}
	% 		\begin{tikzcd}[column sep=8pt,row sep=15pt]
	% 			TTT'X \ar[rr,"\mu T'"] \ar[d,"T \lambda "'] && TT'X \ar[d,"\lambda"]\\
	% 			TT'TX \ar[rd,"\lambda T"'] && T'TX \\
	% 			& T'TTX \ar[ur,"T'\mu"'] &
	% 		\end{tikzcd}
	% 		\end{small}
	% 	\end{center} 
...
\begin{enumerate}[(1)~]
	\item 分配則 $\lambda:FT\Rightarrow TF$ によって誘導される $F$ の \fbox{\textgt{Kleisli extension}} と、\vspace{2pt}
